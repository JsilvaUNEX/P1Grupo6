...
SpecificWorker::RetVal SpecificWorker::wall(auto &filtered_points)
{
    ...
    // Si no se detecta necesidad de giro, reiniciar el contador
...
SpecificWorker::RetVal SpecificWorker::spiral(auto &points)
{
    ...
    spiral_adv_speed = std::min(spiral_adv_speed + 1.7f, MAX_ADV_SPEED);
    spiral_rot_speed = std::max(spiral_rot_speed - 0.001f, MIN_ROT_SPEED);
    ...
    if(min_point != points.end()) {
        qDebug() << "Closest obstacle distance:" << min_point->distance2d;
        if(min_point->distance2d < params.SPIRAL_THRESHOLD) {
            qDebug() << "Obstacle within minimum distance, switching to WALL state.";
            return RetVal(STATE::WALL, 0.f, 0.f);
        }
    }

    qDebug() << "Continuing in SPIRAL mode: Adv Speed =" << spiral_adv_speed << ", Rot Speed =" << spiral_rot_speed;
    // Continuar en modo SPIRAL si no se detectan obstáculos cercanos
